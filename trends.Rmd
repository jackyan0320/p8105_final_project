...
#trend plot for each county
plot examing the year 2012 to look for a change in percentage enrolled slope. It is organized by smallest current percentage to largest
...
organ %>%
  ...
  group_by(county) %>%
  arrange(total_days) %>%
  mutate(last_observation = last(eligible_population_enrolled)) %>% 
  ungroup() %>%
  mutate(county = fct_reorder(county, last_observation)) %>%
...
organ %>% 
  ...
  plot_ly(x = ~eligible_population_enrolled, y = ~population_18_estimate, type = "scatter", mode = "markers",
          alpha = 0.5, 
          text = ~county)
