...
organ_tidy = 
  organ %>% 
  mutate(registry_prop = registry_enrollments / x2012_census_population) %>% 
...
```{r}
ggplot() + 
  geom_polygon(data = ny, aes(x = long, y = lat, group = group), fill = "lightgrey") + 
  geom_polygon(data = ny_county, aes(x = long, y = lat, group = group), fill = NA, color = "white") +
  stat_density2d(data = organ_tidy, aes(x = long, y = lat, fill = ..level.., alpha = ..level..), geom = "polygon") +
